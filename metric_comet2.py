...
df = pd.read_excel("data/esnli_de_gpt4_1_mini_200.xlsx")
...
if system_score is None and seg_scores:
    system_score = sum(seg_scores) / len(seg_scores)

print(f"[ALL] COMET-QE: {system_score:.4f}" if system_score is not None else "[ALL] COMET-QE: NA")

if "COMET_QE" in df_long.columns:
    for name, group in df_long.groupby("field"):
        field_score = group["COMET_QE"].mean()
        print(f"[{name}] COMET-QE: {field_score:.4f}")

# df_long.rename(columns={"src":"original_en","mt":"translation_de"}) \
#       .to_csv("comet_qe_segment_scores.csv", index=False, encoding="utf-8")
